...
void test_send_packet_and_wait_incremented_echo(void)
{
    ...
    int          count          = 0;
    usleep(100);
    if (!node_create(&parent_node, URL_DATA_EXCHANGE_INTER_PROC))
    {
        printf(" PARENT:  error create !\n");
    }
    printf(" PARENT: node created  !\n");
    usleep(1000);
    if (!node_send(&parent_node, &pack, sizeof(radio_packet)))
    {
    ...
    {
        count++;
        if (node_receive(&parent_node, &receive_packet, sizeof(radio_packet)))
        {
...
void test_receive_increment_and_send_back(void)
{
    ...
    int          count                = 0;
    int          timed_out            = 0;
    if (!node_create(&child_node, URL_DATA_EXCHANGE_INTER_PROC))
    {
    ...
    {
        count++;
        if (node_receive(&child_node, &child_receive_packet, sizeof(radio_packet)))
        {
            printf("child  receive \n");
            child_receive_packet.id++;
            if (!node_send(&child_node, &child_receive_packet, sizeof(radio_packet)))
            {
                printf(" CHILD: exit  error send !\n");
            }
            sleep(1);
            if (!node_delete(&child_node))
            {
